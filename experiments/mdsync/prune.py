...
CHANGES = 1024
PRUNING = True
KEY_BITS = 48 # tree depth, should be on the order of lg(SIZE)
...
def h1(x): return md4(SALT1 + hex(x)) & (MAX_KEY-1)
...
async def endpoint(H, rx, tx, archive):
    ...
    while lvl_alive:
        ...
        for vert in sent:
            ...
            if (PRUNING or level == LEVELS-1) and h2(my_val ^ their_val) == my_chk ^ their_chk: # only single chnage in subtree
...
def xfer_stat(archive):
    ...
    record_size = 16*2 if PRUNING else 16
    return record_size * sum( [ len(msg) for msg in archive ] )
...
def reconcile(A, B):
    ...
    print("Comm. rounds:", len(ab_archive))
    print("Net. rountrips:", len(ab_archive)/2)
